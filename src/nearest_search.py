...
from sphero_path_following.msg import OdometryArray
...
class NearestSearch(object):
    ...
    def robot_callback(self, *data):
        ...
        for agent in data[0:self.num_agents1]:
            ...
            for agent2 in data[0:self.num_agents1]:
            ...
            for agent3 in data[self.num_agents1:self.num_agents]:
                friend_position = agent3.pose.pose.position
                distance = get_distance(agent_position, friend_position)
                if distance > 0 and distance <= self.search_radius:
                    # Calculate relative position
                    rel_pos = Pose()
                    rel_pos.position.x = friend_position.x - agent_position.x
                    rel_pos.position.y = friend_position.y - agent_position.y
                    avoids.poses.append(rel_pos)
...
###JURAJ cijeli callback

    def robot_callback2(self, *data):
        """Find and publish positions of nearest agents and obstacles."""

        for agent in data[self.num_agents1:self.num_agents]:
            # Get current time and observed agent's name
            time = rospy.Time.now()
            key = agent.header.frame_id.split('/')[1]
            ...
            # agent and if the distance is within specified, add the agent to
            # the publishing message.
            for agent2 in data[self.num_agents1:self.num_agents]:
                friend_position = agent2.pose.pose.position
                distance = get_distance(agent_position, friend_position)
                if distance > 0 and distance <= self.search_radius:
                    # Calculate relative position
            ...
            for agent3 in data[0:self.num_agents1]:
                friend_position = agent3.pose.pose.position
                distance = get_distance(agent_position, friend_position)
                if distance > 0 and distance <= self.search_radius:
                    # Calculate relative position
    ...
    def __init__(self):
        ...
        self.num_agents1 = rospy.get_param('/num_of_robots1')


        ##jurajev kod 
        self.num_agents2=rospy.get_param('/num_of_robots2')
        ...
        #jurajev kod
        subs2 = [mf.Subscriber(topic_name.format(i), Odometry) for i in range(self.num_agents)]
        self.ts2 = mf.ApproximateTimeSynchronizer(subs2, 10, 0.11)  # TODO: set this to be parametric as well
        self.ts2.registerCallback(self.robot_callback2)
